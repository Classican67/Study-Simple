...
export default async function NotePage(props: PageProps<"/notes/[noteId]">) {
  ...
  const blocks: EditableBlock[] = note.blocks
    ...
    .map((block) => ({ id: block.id, kind: block.kind as EditableBlock["kind"], content: block.content }));
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <ConfirmDialog
          trigger={
            <Button variant="text" size="icon" aria-label="Supprimer la note" className="hover:text-error">
              <Trash2 />
            </Button>
          }
      ...
      <NoteEditor noteId={note.id} initialTitle={note.title} initialBlocks={blocks} />
    </div>
  );
}
